...
def parse_arguments(verbose=True):
    ...
    parser.add_argument(
        "-xyzfile",
        help="XYZ file containing reactant and, if DE-GSM, product.",
        required=True,
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "-command", type=str, default="", help="comamnd for running the package"
    )
    parser.add_argument(
        "-lot_inp_file",
        type=str,
    ...
    parser.add_argument(
        ...
        type=int,
        default=[0],
        help="Adiabatic index (default: %(default)s)",
        required=False,
    )
    parser.add_argument(
        "-multiplicity",
        ...
        type=int,
        default=[1],
        help="Multiplicity (default: %(default)s)",
    )
    parser.add_argument(
        "-FORCE_FILE",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Harmonic translational restraints",
    )
    parser.add_argument(
        "-optimizer",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        default="GFN2-xTB",
        help="xTB hamiltonian",
        choices=["GFN2-xTB", "GFN1-xTB"],
        required=False,
    )
    parser.add_argument(
        "-xTB_accuracy", type=float, default=1.0, help="xTB accuracy", required=False
    )
    parser.add_argument(
        "-xTB_electronic_temperature",
        type=float,
        default=300.0,
        help="xTB electronic temperature",
        required=False,
    )
    parser.add_argument(
        "-xyz_output_format",
    ...
    parser.add_argument(
        "-solvent",
        type=str,
        help="Solvent to use (xTB calculations only)",
        required=False,
    )
    parser.add_argument(
        "-linesearch",
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.8,
        help="Maximum step size in single-ended mode (default: %(default)s)",
        required=False,
    )
    parser.add_argument(
        "-BDIST_RATIO",
    ...
    parser.add_argument(
        "-optimize_mesx", action="store_true", help="optimize to the MESX"
    )
    parser.add_argument(
        "-optimize_meci", action="store_true", help="optimize to the MECI"
    )
    ...
    parser.add_argument(
        "-dont_analyze_ICs",
        action="store_false",
        help="Don't post-print the internal coordinates primitives and values",
    )  # defaults to true
    parser.add_argument(
        "-hybrid_coord_idx_file",
        type=str,
        default=None,
        help="A filename containing a list of  indices to use in hybrid coordinates. 0-Based indexed",
    )
    parser.add_argument(
        "-frozen_coord_idx_file",
        type=str,
        default=None,
    ...
    parser.add_argument(
        "-bonds_file", type=str, help="A file which contains the bond indices (0-based)"
    )
    parser.add_argument(
    ...
    group_ase.add_argument(
        "--ase-kwargs",
        type=str,
        help="ASE calculator keyword args, as JSON dictionary, "
...
def run_gsm():
    ...
    # LOT
    nifty.printcool(
        "Build the {} level of theory (LOT) object".format(inpfileq["EST_Package"])
    )
    ...
    if (
        inpfileq["optimize_mesx"]
        or inpfileq["optimize_meci"]
        or inpfileq["gsm_type"] == "SE_Cross"
    ):
        assert (
            inpfileq["PES_type"] == "Penalty_PES"
        ), "Need penalty pes for optimizing MESX/MECI"
    ...
    if inpfileq["hybrid_coord_idx_file"] is not None:
        ...
        assert (
            inpfileq["coordinate_type"] == "TRIC"
        ), "hybrid indices won't work (currently) with other coordinate systems"
    ...
    if inpfileq["prim_idx_file"] is not None:
        ...
        assert (
            inpfileq["coordinate_type"] == "TRIC"
        ), "won't work (currently) with other coordinate systems"
        prim_indices = np.loadtxt(inpfileq["prim_idx_file"])
        if prim_indices.ndim == 2:
            prim_indices = [
                (int(prim_indices[i, 0]), int(prim_indices[i, 1]) - 1)
    ...
    if not inpfileq["reactant_geom_fixed"] and inpfileq["gsm_type"] != "SE_Cross":
        ...
        optimizer.optimize(
            molecule=reactant, refE=reactant.energy, opt_steps=100, path=path
        )

    if not inpfileq["product_geom_fixed"] and inpfileq["gsm_type"] == "DE_GSM":
        path = os.path.join(
            os.getcwd(),
            "scratch/{:03}/{}/".format(inpfileq["ID"], inpfileq["num_nodes"] - 1),
        )
        nifty.printcool("PRODUCT GEOMETRY NOT FIXED!!! OPTIMIZING")
        optimizer.optimize(
            molecule=product, refE=reactant.energy, opt_steps=100, path=path
        )
